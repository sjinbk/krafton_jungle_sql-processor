...
그 과정에서 하네스 엔지니어링 개념을 접했습니다. 하네스 엔지니어링은 AI 에이전트를 감싸는 구조적 계층 — 도구 접근 제어, 검증 게이트, 상태 관리, 세션 간 핸드오프 — 을 설계하는 분야입니다. 세션이 끊겨도 진행 상태를 구조화된 문서로 넘겨 연속성을 확보하는 것은 그 핵심 기법 중 하나입니다.
...
### 평가 기준
구조, 가독성, 설계 품질 등의 정적 분석 후 아래 케이스로 실행 기반 평가
- T1: 기본 INSERT 1건 후 SELECT → row 1개 출력 확인
- T2: INSERT 3건 후 SELECT → row 3개 출력 확인
- T3: 빈 테이블 SELECT → 빈 결과 또는 적절한 메시지 확인
- T4: 잘못된 SQL 입력 → 에러 처리 확인
